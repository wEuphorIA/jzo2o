...
public class ServeItemServiceImpl extends ServiceImpl<ServeItemMapper, ServeItem> implements IServeItemService {
    ...
    public void deactivate(Long id) {
        ...
        long count = serveService.count(new LambdaQueryWrapper<Serve>().eq(Serve::getServeItemId, id).eq(Serve::getSaleStatus, FoundationStatusEnum.ENABLE.getStatus()));
        if (count > 0) {
            throw new ForbiddenOperationException("有区域在使用该服务项，请先解除关联关系");
        }
